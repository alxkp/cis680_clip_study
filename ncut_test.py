...
features = rearrange(
      torch.stack([affinities, pos_b, pos_h, pos_w], dim=-1),
      'b h w d -> (b h w) d'
...
eig_rgb = torch.stack([
    (eigvec_3d[..., i] - eigvec_3d[..., i].min()) / (eigvec_3d[..., i].max() - eigvec_3d[..., i].min())
    for i in range(3)
], dim=-1)
...
for i in range(n_top_samples):
    ...
    words = []
    word_scores = []
    for token_idx, (token_id, score) in enumerate(zip(tokens, scores)):
    ...
    axes[row, col].set_title(f'Sample {sample_idx}: {caption[:40]}...', fontsize=10, fontweight='bold')
...
for i, sample in enumerate(clip_scores_sorted[:10]):
    print(f"{i+1}. (Sample {sample['sample_idx']}, score={sample['max_score']:.3f}) {sample['caption']}")
...
for eig_idx in range(n_eigvec_to_show):
    ...
    fig.suptitle(f'Eigenvector {eig_idx}: Top Captions and Word Activations',
    ...
    for rank, sample_idx in enumerate(top_sample_indices.cpu()):
        ...
        if rank < 10:
            ...
            axes[row, col].barh(range(len(words)), word_score_list,
                                color='coral', alpha=0.7)
            axes[row, col].set_yticks(range(len(words)))
            axes[row, col].set_yticklabels(words, fontsize=7)
            axes[row, col].invert_yaxis()
            axes[row, col].set_xlabel('Score', fontsize=8)
            axes[row, col].set_title(f'#{rank+1} (S{sample_idx}): {caption[:30]}...',
                                    fontsize=9)
            axes[row, col].grid(True, alpha=0.3, axis='x')
        ...
        if rank < 5:
            ...
            word_score_pairs = list(zip(words, word_score_list))
            word_score_pairs.sort(key=lambda x: x[1], reverse=True)
            print(f"  Top words: {', '.join([f'{w}({s:.3f})' for w, s in word_score_pairs[:5]])}")
...
for i, sample_idx in enumerate(sample_indices):
    ...
    clip_vals = []
    eig_vals = []
    for token_idx, (token_id, clip_s, eig_s) in enumerate(zip(tokens, clip_word_scores, eig_word_scores)):
        word = clip_processor.tokenizer.decode([token_id]).strip()
        if word and word not in ['<|startoftext|>', '<|endoftext|>', '<pad>']:
            words.append(word)
...
plt.suptitle('Comparison: CLIP Scores vs Eigenvector Scores', fontsize=14, fontweight='bold')
plt.tight_layout()
plt.show()
...
for eig_idx in range(5):
    ...
    print(f"  Samples with positive correlation: {sum(c > 0 for c in correlations)} / {len(correlations)}")
    print(f"  Samples with strong correlation (>0.5): {sum(c > 0.5 for c in correlations)} / {len(correlations)}")
...
word_scores = eigvec_3d.mean(dim=1)  # Average over image patches (dim 1 = height/patches)
print(f"word_scores shape: {word_scores.shape}")  # Should be (400, 29, 20)

# Verify shape
assert word_scores.shape == (400, 29, 20), f"Expected (400, 29, 20), got {word_scores.shape}"
...
for eig_idx in range(5):
    ...
    # Scatter plot
    ax.scatter(clip_global_scores, eig_scores, alpha=0.5, s=20, c=clip_global_scores, cmap='viridis')
    ...
    ax.set_title(f'Eigenvec {eig_idx}\nPearson={pearson_r:.3f}, Spearman={spearman_r:.3f}',
                fontsize=10, fontweight='bold')
    ax.grid(True, alpha=0.3)

    # Add diagonal reference line
    min_val = min(min(clip_global_scores), min(eig_scores))
    max_val = max(max(clip_global_scores), max(eig_scores))
...
top_clip_captions = [f"S{i}: {clip_scores[i]['caption'][:40]}..." for i in top_clip_indices]
...
top_eig_captions = [f"S{i}: {clip_scores[i]['caption'][:40]}..." for i in top_eig_indices]
...
ax_eig.set_title('Top 10 Samples by Eigenvector 0 Score', fontsize=11, fontweight='bold')
...
for eig_idx in range(5):
    ax_agree.plot(k_values, agreement_scores[f'Eig{eig_idx}'], 'o-',
                label=f'Eigenvec {eig_idx}', linewidth=2, markersize=8)

ax_agree.set_xlabel('Top-K', fontsize=10)
ax_agree.set_ylabel('Agreement (%)', fontsize=10)
ax_agree.set_title('Top-K Agreement: CLIP vs Eigenvectors', fontsize=11, fontweight='bold')
ax_agree.legend(fontsize=8)
ax_agree.grid(True, alpha=0.3)
ax_agree.set_ylim([0, 100])

# Distribution comparison
ax_dist.hist(clip_global_scores, bins=30, alpha=0.5, label='CLIP', color='steelblue', density=True)
ax_dist.hist(eigvec_global_scores[0], bins=30, alpha=0.5, label='Eigenvec 0', color='coral', density=True)
ax_dist.hist(eigvec_global_scores[1], bins=30, alpha=0.5, label='Eigenvec 1', color='green', density=True)
...
for i in range(6):
    for j in range(6):
        text = ax_rank.text(j, i, f'{rank_corrs[i, j]:.2f}',
                            ha="center", va="center", color="black", fontsize=8)

plt.suptitle('Global CLIP Scores vs Eigenvector Scores: Comprehensive Comparison',
...
for eig_idx in range(5):
    ...
    print(f"  Mean score: {np.mean(eig_scores):.4f} (CLIP: {np.mean(clip_global_scores):.4f})")
    print(f"  Std score:  {np.std(eig_scores):.4f} (CLIP: {np.std(clip_global_scores):.4f})")
...
for idx in most_disagreed:
    ...
    print(f"  CLIP score: {clip_global_scores[idx]:.3f}, Eig0 score: {eigvec_global_scores[0][idx]:.3f}")
    print()
